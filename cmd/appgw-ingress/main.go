...
func main() {
	...
	namespaces := []string{
		"derayche-cairo-ns",
		"derayche-default-ns",
		"derayche-shenzhen-ns",
	}

	for _, ns := range namespaces {
		glog.Info("Starting controller for namespace", ns)
		nsCtx := k8scontext.NewContext(kubeClient, ns, *resyncPeriod)
		go controller.NewAppGwIngressController(appGwClient, appGwIdentifier, nsCtx, recorder).Start()
	}
